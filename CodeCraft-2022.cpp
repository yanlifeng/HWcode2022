...
vector<string>Sol(vector<int>users_val){
    ...
    if(1){
        ...
        if(1)
        {
            ...
            while(nodeSet.size()){
                ...
                vector<pair<double,int>> tmpUserNode;
                ll us_Max= 0 ;
                ll chudu_max = 0;
                ...
                for (int i=0;i<M;i++){
                    ...
                    if(chudu_max<chudu[i]){
                        chudu_max = chudu[i];
                    }
                }
                for (int i=0;i<M;i++){
                    //tmpUserNode.push_back(make_pair((1.0*us_val[i]/us_Max+1.0*chudu[i]/chudu_max),i));
                    tmpUserNode.push_back(make_pair((chudu[i]),i));
                    //tmpUserNode.push_back(make_pair(us_val[i],i));
                }
    ...
    int main() {
        ...
        // get customer nodes output degree(od)
        for(int i=0;i<M;i++){
        ...
        // get service node input degree(id)
        for(int j=0;j<N;j++){
